...
int main(void) {
    vals = (stack *) malloc(sizeof(stack));
    oprts = (stack *) malloc(sizeof(stack));
    init(vals);
    init(oprts);
    char infix[N];
    char postfix[N];
        ...
        if (is_digit(c)) {
           ...
           postfix[index++] = ' '; 
            ...
            if (is_empty(oprts) || c == '(') {
                ...
                char oprt = pop(oprts)->element;
                while (oprt != '(') {
                    postfix[index++] = oprt;
                    oprt = pop(oprts)->element;
                }
            } else {
                if (priority(c) > priority(peek(oprts)->element)) {
                    // 当前操作符的优先级大于栈顶操作符的优先级，直接入栈
                    push(oprts, c);
                } else {
                    // 否则将操作符栈中的操作符弹出，直到遇到比当前操作符优先级小的操作符，并把当前操作符入栈
                    while ((! is_empty(oprts)) && (priority(c) <= priority(peek(oprts)->element))) {
                        postfix[index++] = pop(oprts)->element;
                    }
                    push(oprts, c);
                }
            }
...
int evaluate(char postfix[]) {
    ...
    for (int i=0;i<len;i++) {
        ...
        if (is_digit(c)) {
            ...
            int b = pop(vals)->element;
            int a = pop(vals)->element;
            int result = computing(a, b, c);
            push(vals, result);
        }
    }
    return pop(vals)->element;
}
